...
- **ドメインモデルは構造体として定義する。** プリミティブな型に別名を与える形（`type Weather int` のような named type）は採らない。単一の値しか持たないモデルであっても、その値を非公開フィールドとして構造体に包む。named type ではパッケージ外から `Weather(200)` のような直接変換ができてしまい、生成関数に置いた範囲検証や解釈を迂回できる。構造体かつフィールド非公開であれば、値の生成経路が生成関数に限定され、型を保持していること自体が不変条件を満たしていることの保証になる。フィールドが比較可能な型だけで構成される限り構造体も比較可能であり、`==` による等価性は失われない。

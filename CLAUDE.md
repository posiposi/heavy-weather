...
**必要になった時点で実装する（KISS）。** 将来使うだろうという見込みで振る舞いを追加しない。追加してよいのは、それを呼ぶコードを同じ変更の中で書けるときに限る。

呼び出し元が無い実装は、正しいかどうかを検証できない。引数の型・戻り値・命名が使いやすいかは、実際に呼ぶコードを書いて初めて判断できる。呼び出し元が後から現れたときには、先に書いた形に合わせて呼び出し側が歪むか、結局書き直すことになる。テストがあっても状況は変わらない。そのテストは実装が想定どおり動くことしか示さず、その想定が正しいことは示さない。

Issue に「やること」として挙がっている項目でも、着手時点で呼び出し元を書けないと判明した場合は実装せず、Issue を分割する。仕様が先に決まっていることと、いま実装すべきであることは別である。

判断の目安。

- **呼び出し元を同じ PR で書けるか。** 書けないなら、その振る舞いは呼び出し元と一緒に別 Issue で実装する
- **公開する必要があるか。** パッケージ外から使われる予定が無いものは公開しない。アクセサを予防的に生やさない
- **抽象化は2つ目の実装が現れてから入れる。** 実際に必要になるまでインターフェースを作らない（`.claude/rules/go-style.md`）

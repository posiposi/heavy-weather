...
**マイグレーションにはテーブル設計だけを書く。** ストレージエンジン・文字セット・照合順序のようにサーバ側のデフォルトで決まる指定は書かない（デフォルト値は `compose.yaml` の `db` サービスと IaC のパラメータグループで揃える）。カラム定義・制約・インデックスなど、そのテーブル固有の設計だけを記述する。

Go の alpine イメージは使わない。musl libc であり、本番の Lambda ランタイム `provided.al2023`（glibc）と揃わないため。

## ブランチ運用

**`main` ブランチでは作業を行わない。** ファイルの作成・編集・削除を伴う変更は、必ず作業ブランチを切ってから始める。これはコードに限らず、マイグレーション・設定ファイル・ドキュメントの変更も対象である。

着手前に現在のブランチを確認し、`main` にいる場合は作業ブランチへ切り替える。

```sh
git branch --show-current
git switch main && git pull origin main
git switch -c feature/<Issue番号>-<内容の要約(kebab-case)>
```

Issue を起点としない作業では `<Issue番号>-` を省き、内容がわかる名前にする。

守るべき点。

- **1行でも書き換える前に切る。** 「後でブランチに移せばよい」と考えて `main` で書き始めない。レビューや検証の途中で `main` が汚れていることに気づくと、退避と切り分けに手間がかかる
- **既に `main` 以外にいる場合は切り直さない。** 同一タスクの修正ラウンドで新しいブランチを作ると、変更が複数ブランチに分散する
- **エージェント経由の実装でも同じ。** `implement` スキルは Phase 0 でブランチを作成するが、`implement-team` スキルや `implementer` エージェントを直接起動した場合はその前段がない。起動する側がブランチを用意すること
...
- **外部 API の `null` は Adapter 層で弾く。** `internal/domain` の生成関数はポインタを受け取らず、値が存在する場合のみを引数に取る。値が存在しないことは値の一種ではないため、欠損を値オブジェクトの内部状態として持たせない。ドメインの型を保持していること自体が値の存在を保証する形にすることで、利用側が「値が入っているか」を確認する義務を負わずに済む。欠損時に当該データだけを落とすか予報全体を欠測として扱うかは `internal/usecase` の判断であり、外部 API 障害時は部分成功を許容するという方針に従って決める。
- **ドメインモデルは構造体として定義する。** プリミティブな型に別名を与える形（`type Weather int` のような named type）は採らない。単一の値しか持たないモデルであっても、その値を非公開フィールドとして構造体に包む。named type ではパッケージ外から `Weather(200)` のような直接変換ができてしまい、生成関数に置いた範囲検証や解釈を迂回できる。構造体かつフィールド非公開であれば、値の生成経路が生成関数に限定され、型を保持していること自体が不変条件を満たしていることの保証になる。
- **生成関数は成功時にポインタを、失敗時に `nil` と `error` を返す。** ゼロ値を返す形では、`error` を確認せずに戻り値を使った誤用が静かに通る。降水確率の `0`（0%）のようにゼロ値が正常値と区別できない型では、誤った判定に進んだことに気づけない。`nil` を返せば同じ誤用が `nil` デリファレンスとして即座に露見する。引数でポインタを受け取らない方針（上記「外部 API の `null` は Adapter 層で弾く」）は維持し、`nil` が意味を持つのは戻り値の側だけとする。等価性はポインタの `==` ではアドレス比較になるため、値を取り出して比較する。

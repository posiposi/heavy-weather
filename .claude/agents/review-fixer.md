...
- ドメイン層（`internal/domain`）の不変性、および `aws-lambda-go` / AWS SDK を import していないこと
- 依存関係の方向（依存性逆転の原則。インターフェースはドメイン／ユースケース側、実装は `internal/adapter`）
- 層の責務分離（`internal/domain` / `internal/usecase` / `internal/adapter` / `cmd`）
- 命名規則の遵守
- `CLAUDE.md` の「アーキテクチャ上の不変条件」に反していないこと

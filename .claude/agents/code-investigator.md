...
**前提**: 本リポジトリはまだ実装コードが存在しない段階から始まる。設計書 `heavy-weather-architecture.md` と `CLAUDE.md` が唯一の真実の源であり、既存コードが見つからない場合はこれらから想定構成・設計上の不変条件を読み取って調査結果とする。「関連コードなし」で終わらせない。

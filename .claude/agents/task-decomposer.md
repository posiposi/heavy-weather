...
- 既存コードが無い（未実装）領域の場合は、設計書 `heavy-weather-architecture.md` の想定構成（§8）と実装ステップ（§9）を基準に分解する
...
7. **設計書の段階順を尊重する**: `heavy-weather-architecture.md` §9 の実装ステップ（第0〜第5段階）を飛ばす分解をしない。段階を飛ばすと問題の切り分けが困難になる

...
> **前提**: 本リポジトリは git 初期化・リモートリポジトリの作成が未実施の可能性がある。その場合 PR 作成そのものが成立しないため、本スキルは**タイトル・本文の草案生成のみ**を行い、呼び出し元にその旨を伝える。Issue が存在しない場合は `Closes #[Issue番号]` 行を省略する。
...
- 良い例: `降水確率の連続時間帯をまとめる判定ロジックを実装`
- 悪い例: `#12 降水確率の連続時間帯をまとめる判定ロジックを実装`
- 悪い例: `降水確率の連続時間帯をまとめる判定ロジックを実装しました（日をまたぐ降雨・閾値ちょうど・データ欠損の境界値対応を含む）`
...
- PR作成は呼び出し元フロー（`implement` スキルの Phase 4）のメインコンテキストが **`/commit-commands:commit-push-pr`** を使用して実行する
